...
async function build() {
  ...
  summon.once('close', () => {
    git({ cwd: __dirname, stdio: 'ignore' }, 'add', 'dist').then(() => {
      git({ cwd: __dirname, stdio: 'ignore' }, 'commit', '-m', 'build ' + new Date());
    });
  });
}
...
function updateVersion() {
  if (!process.env['GITHUB_WORKFLOW']) {
    exec('git describe --tags --first-parent --dirty --broken', { cwd: __dirname }, function (err, hash) {
...
function git(options: null | SpawnOptions = {}, ...args: string[]) {
  return new Promise(
    (
      resolve: (args: { code: number | null; stdout: string | null; stderr: string | null }) => any,
...
function getLatestCommitHash(path?: string) {
  return new Bluebird((resolve: (result: { err: ExecException | null; hash: string | null }) => any) => {
    if (path) {
      exec(`git log --pretty=tformat:"%h" -n1 ${path}`, { cwd: __dirname }, (err, hash) => {
        resolve({ err, hash: String(hash).trim() });
      });
    } else {
      exec(`git log --pretty=tformat:"%h" -n1 .`, { cwd: __dirname }, (err, hash) => {
        resolve({ err, hash: String(hash).trim() });
      });
    }
  });
