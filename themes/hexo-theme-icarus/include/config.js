...
function checkConfig(hexo) {
  if (!process.argv.includes('--icarus-dont-check-config')) {
    ...
    const themeCfgPaths = [themeDirCfg, themeSiteCfg].filter((cfgPath) =>
    ...
    const loader = SchemaLoader.load(
      require(path.join(schemaDir, 'config.json')),
      schemaDir
    );
    ...
    if (!process.argv.includes('--icarus-dont-generate-config')) {
      if (!themeCfgPaths.length) {
        ...
        logger.info(
          'To skip configuration generation, use "--icarus-dont-generate-config".'
        );
      }
    }
    ...
    if (!process.argv.includes('--icarus-dont-upgrade-config')) {
      const migrator = new Migrator(
        require(path.join(hexo.theme_dir, 'include/migration/head'))
      );
      if (cfg.version && migrator.isOudated(cfg.version)) {
        logger.warn(
          `Your theme configuration is outdated (${
            cfg.version
          } < ${migrator.getLatestVersion()}).`
        );
        logger.info(
          'To skip the configuration upgrade, use "--icarus-dont-upgrade-config".'
        );
        ...
        logger.info(
          `Theme configurations are written to ${yellow(themeSiteCfg)}.`
        );

        generateThemeConfigFile(schema, themeSiteCfgExample);
        logger.info(
          `Example configurations is at ${yellow(themeSiteCfgExample)}.`
        );
      }
    }
    ...
    if (validation !== true) {
      ...
      logger.warn(
        'Icarus may still run, but you will encounter unexcepted results.'
      );
      logger.warn(
        'Here is some information for you to correct the configuration file.'
      );
    ...
    if (rootCfg.theme_config) {
      ...
      logger.warn(
        `Please remove theme configurations from ${yellow(siteCfgFile)}.`
      );
    }
  }
}
...
module.exports = (hexo) => {
  try {
    ...
    logger.info(
      "You may use '--icarus-dont-check-config' to skip configuration checking."
    );
    process.exit(-1);
  }
};
