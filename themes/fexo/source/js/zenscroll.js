  ...
  var createScroller = function (scrollContainer, defaultDuration, edgeOffset) {
    ...
    var getRelativeTopOf = function (elem) {
      if (scrollContainer) {
        ...
        return (
          elem.getBoundingClientRect().top + getScrollTop() - docElem.offsetTop
        );
      }
    };
      ...
      if (nativeSmoothScrollEnabled()) {
        ...
        (function loopScroll() {
          scrollTimeoutId = setTimeout(function () {
            ...
            var y = Math.max(
              Math.floor(
                startY + distance * (p < 0.5 ? 2 * p * p : p * (4 - p * 2) - 1)
              ),
              0
            );
            if (scrollContainer) {
            ...
            if (
              p < 1 &&
              getViewHeight() + y < (scrollContainer || docElem).scrollHeight
            ) {
              loopScroll();
            } else {
    ...
    var scrollIntoView = function (elem, duration) {
      var elemScrollHeight =
        elem.getBoundingClientRect().height + 2 * edgeOffset;
