  ...
  const run = gulp
    ...
    .pipe(
      through2.obj(function (file, _encoding, next) {
        ...
        if (modify === null || typeof modify === 'undefined')
          modify = modifyPost(parse, null, false);
        ...
        parse.metadata.category.forEach((name) => {
          ...
          if (
            !postCats[name].find(({ title }) => title === parse.metadata.title)
          )
            postCats[name].push(parse);
        });
        parse.metadata.tags.forEach((name) => {
          ...
          // prevent duplicate push
          if (
            !postTags[name].find(({ title }) => title === parse.metadata.title)
          )
            postTags[name].push(parse);
        });
