...
class logic extends Hexo {
  constructor(data) {
    ...
    this.config = Object.assign(defConfig, this.config);
    ...
    var overwrite = true;
    if (
      this.config.hasOwnProperty("auto_category") &&
      this.config.auto_category.hasOwnProperty("enable") &&
      this.config.auto_category.enable &&
      overwrite
    ) {
      let postStr;
