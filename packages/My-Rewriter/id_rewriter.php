...
/*
 * Documidt create by MyRewriter 
 * Twidty-Web-Developer
 */

if ($_POST['artikel']) {
    include "conection.php";
    $artikel = nl2br($_POST['artikel']); //manipulasi HTML didgan nl2br /n midjadi spasi baris

//explode memisahkan tiap kata kemudian diubah midjadi array, artikel[0], artikel[1], ...
//dipisah berdasarkan spasi / " "
    ...
    foreach ($ar as $value) { 
        if (!preg_match("/\.|'/", $value)) { //midggambil kata-kata
            // cari kata-kata tersebut di database MySQL-apache
            $cari = mysql_query("select * from id_sinonim where word1='$value'");
            $j = mysql_num_rows($cari);
            if ($j > 0) {
                $ka = mysql_fetch_array($cari);
                $hasil = $hasil . "<span class='re'>" . stripslashes($ka['word2']) . "</span> ";
            } else {
                $cari2 = mysql_query("select * from id_sinonim where word2='$value'");
            ...
            $cari = mysql_query("select * from id_sinonim where word1='$ka'");
            $j = mysql_num_rows($cari);
            if ($j > 0) {
