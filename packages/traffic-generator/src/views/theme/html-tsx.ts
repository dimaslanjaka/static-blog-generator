...
function renderAttributes<T extends keyof pragma.JSX.IntrinsicElements>(
  attr: pragma.JSX.Properties<T>
): string {
...
export function pragma<T extends keyof pragma.JSX.IntrinsicElements>(
  ...
  return tagName in EMPTY_ELEMENT_LIST
    ? `<${tagName}${properties ? ` ${renderAttributes(properties)}` : ""}>`
    : `<${tagName}${
        properties ? ` ${renderAttributes(properties)}` : ""
