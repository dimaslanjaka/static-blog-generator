  ...
  ldloader = function (opt) {
    ...
    this.root.map(function (it) {
      it.classList.add.apply(
        it.classList,
        (self.opt.className || "").split(" ").filter(function (it) {
          return it;
        })
      );
  ...
  ldloader.prototype = import$(Object.create(Object.prototype), {
    ...
    toggle: function (v, delay, force) {
      ...
      ret = new Promise(function (res, rej) {
        ...
        self.running = running = self.root[0].classList.contains(
          self.opt.activeClass
        );
        if (self.opt.ctrl) {
          self.render();
        }
  ...
  import$(ldloader, {
    _zmgr: {
      add: function (v) {
        var z, ref$;
        (this.s || (this.s = [])).push(
          (z = Math.max(v || 0, ((ref$ = this.s)[ref$.length - 1] || 0) + 1))
        );
        return z;
      },
      remove: function (v) {
