...
class Ripple {
    constructor(element, options) {
        ...
        if (this._element) {
            ...
            manipulator_1.default.addClass(this._element, CLASSNAME_RIPPLE);
        }
    ...
    _appendRipple(target, parent) {
        ...
        setTimeout(() => {
            manipulator_1.default.addClass(target, 'active');
        }, FIX_ADD_RIPPLE_EFFECT);
    }
    _toggleUnbound(target) {
        if (this._options.rippleUnbound === true) {
            manipulator_1.default.addClass(target, CLASSNAME_UNBOUND);
        }
        else {
    ...
    _addColor(target, parent) {
        ...
        if (IS_BOOTSTRAP_COLOR) {
            manipulator_1.default.addClass(parent, `${CLASSNAME_RIPPLE}-${this._options.rippleColor.toLowerCase()}`);
        }
        else {
 ...
 */
SELECTOR_COMPONENT.forEach((selector) => {
    event_handler_1.default.one(document, 'mousedown', selector, Ripple.autoInitial(new Ripple()));
});
/**
 * ------------------------------------------------------------------------
 * jQuery
