...
window.onload = function () {
    electron_1.ipcRenderer.on("session-error", function (evt, message) {
    ...
    webviews.forEach((webview) => {
        ...
        function reloadWebProxy(partisi) {
            electron_1.ipcRenderer
                .invoke("change-webview-proxy", partisi)
                .then((proxyResult) => {
                const webviewContainer = document.querySelector('[data-partition="' + partisi + '"]');
                if (webviewContainer) {
        ...
        webview.addEventListener("did-finish-load", (e) => {
            ...
            // skip cloudflare
            if (/^please.*wait.*cloudflare$|gateway.*timeout$/gm.test(titlePage.toLowerCase())) {
                const partisi = webview.getAttribute("partition");
                reloadWebProxy(partisi);
            }
            else {
...
//# sourceMappingURL=webview-control.js.map