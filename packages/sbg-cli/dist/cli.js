...
yargs_1.default
  ...
  .command(
    ...
    function (yargs) {
      ...
      yargs.positional(`archive`, {
        type: `string`,
        describe: `clean categories, tags, archives inside ${env_1.rootColor}/.deploy_${
          api.config.deploy.type || 'git'
        }`
      });
  ...
  .command(
    'post <key>',
    `operation inside ${env_1.rootColor}/${api.config.post_dir}`,
    function (yargs) {
  ...
  .command(
    'generate <key>',
    `generate operation on ${env_1.rootColor}/${api.config.public_dir}`,
    function (yargs) {
    ...
    async function ({ key }) {
      switch (key) {
        ...
        case 'hexo':
          ...
          await (0, git_command_helper_1.spawnAsync)('npx', ['hexo', 'generate'], {
            cwd: api.cwd,
            stdio: 'inherit',
            shell: true
          });
  ...
  .command(
    'deploy <key>',
    `operation inside ${env_1.rootColor}/.deploy_${api.config.deploy?.type || 'git'}`,
    function (yargs) {
      yargs.positional(`seo`, {
        type: `string`,
        describe: `fix seo`
