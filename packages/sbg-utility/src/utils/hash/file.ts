  ...
  if (fs.existsSync(folder)) {
    ...
    for (const item of matches) {
      ...
      if (statInfo.isFile()) {
        ...
        filesWithHash[fullPath] = dataToHashSync(algorithm, fileInfo, options.encoding);
      }
    }

    return {
      filesWithHash,
      hash: dataToHashSync(algorithm, Object.values(filesWithHash).join(''), options.encoding)
    };
  }
