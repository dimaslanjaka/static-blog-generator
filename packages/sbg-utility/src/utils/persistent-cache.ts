...
export class persistentCache implements PersistentCacheOpt {
	...
	get<T>(
		name: string,
		fallback?: (e: Error | undefined | null, entry?: T) => any
	): void;
	...
	keysSync() {
		...
		if (fs.existsSync(this.getCacheDir())) {
			return fs
				.readdirSync(this.getCacheDir())
				.map(self.transformFileNameToKey);
		} else {
			return [];
		}
	}
	...
	buildCacheEntry(data: any) {
		...
		return {
			cacheUntil: !cacheInfinitely
				? new Date().getTime() + this.duration
				: undefined,
			data: data,
		};
	}
