...
function md5FileSync(path) {
    ...
    const hashSum = crypto__default.createHash('md5'); // sha256
 ...
 */
function md5(data) {
    return crypto__default.createHash('md5').update(data).digest('hex');
}
function md5File(path) {
    return new Promise((resolve, reject) => {
        const output = crypto__default.createHash('md5');
...
function file_to_hash(alogarithm, path, encoding = 'hex') {
    return new Promise((resolve, reject) => {
        const hash = crypto__default.createHash(alogarithm);
...
function data_to_hash_sync(alogarithm = 'sha1', data, encoding = 'hex') {
    return crypto__default.createHash(alogarithm).update(data).digest(encoding);
}
/**
...
export { data_to_hash, data_to_hash_sync, md5File as default, file_to_hash, folder_to_hash, md5, md5FileSync, url_to_hash };
