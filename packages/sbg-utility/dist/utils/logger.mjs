...
import '../config/config-wrapper.mjs';
import '../config/default-config.mjs';
import 'path';
import 'bluebird';
import 'minimatch';
import './filemanager/case-path.mjs';
import { writefile } from './filemanager/writefile.mjs';
import { areWeTestingWithJest } from './jest.mjs';

let FOLDER = path__default.join(process.cwd(), 'tmp/logs');
let cwd = process.cwd();
/*
declare global {
...
if (areWeTestingWithJest()) {
    ...
    console.log = function (...args) {
        if (typeof getConfig === 'function') {
            const cfg = getConfig();
            FOLDER = path__default.join(cfg.cwd, 'tmp/logs/');
            cwd = cfg.cwd;
        }
        ...
        // log(stack[2], stack[4]);
        const filename = slugify(path__default.toUnix(msg).replace(path__default.toUnix(cwd), ''), {
            lower: true,
            trim: true,
            replacement: '-',
            strict: true
        ...
        const header = `\n\n ${new Date()} \n\n`;
        const write = writefile(path__default.join(FOLDER, filename + '.log'), header + args.join('\n\n'), { append: true });
        log(write.file);
    };
}
    ...
    static tracer(...args) {
        ...
        if (split) {
            ...
            if (typeof split[0].path === 'undefined' && split[1].path.includes('anonymous')) {
                ...
                const base = path__default.basename(filePath.split(':')[0].length === 1
                    ? filePath.split(':')[0] + ':' + filePath.split(':')[1]
                    : filePath.split(':')[0]);
                logfile = path__default.join(FOLDER, slugify(id, { trim: true }) + '-' + slugify(base, { trim: true }) + '.log');
                ...
                fs__default.appendFileSync(logfile, templ);
            }
            // Logger.log(split);
        }
    }
