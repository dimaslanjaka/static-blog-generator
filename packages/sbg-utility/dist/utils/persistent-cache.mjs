...
import { normalizePath } from './filemanager/normalizePath.mjs';
...
class persistentCache {
    ...
    constructor(options = {}) {
        this.base =
            options.base ||
                normalizePath((require.main ? path__default.dirname(require.main.filename) : undefined) || process.cwd(), 'tmp');
    ...
    getSync(name, fallback) {
        ...
        let data;
        try {
            data = JSON.parse(fs__default.readFileSync(this.buildFilePath(name), 'utf8'));
        }
        catch (_e) {
            return fallback;
        }
    ...
    get(name, fallback) {
        if (typeof fallback !== 'function') {
            ...
            fs__default.readFile(this.buildFilePath(name), 'utf8', function (err, content) {
                if (err != null) {
                    return safeCb(fallback)(null, undefined);
                }
                let entry;
    ...
    deleteEntry(name, cb) {
        ...
        fs__default.unlink(this.buildFilePath(name), cb);
    }
    /**
     * delete cache sync
     * @param name cache key
    ...
    deleteEntrySync(name) {
        ...
        fs__default.unlinkSync(this.buildFilePath(name));
    }
    getCacheDir() {
        return path__default.normalize(this.base + '/' + (this.name || 'cache'));
    }
    /**
    ...
    unlink(cb) {
        if (this.persist) {
            if (typeof cb !== 'function') {
                return fs__default.rmSync(this.getCacheDir(), { recursive: true, force: true });
            }
            else {
                return fs__default.rm(this.getCacheDir(), { recursive: true, force: true }, safeCb(cb));
            }
        }
        safeCb(cb)(null);
    }
    ...
    keys(cb) {
        ...
        fs__default.readdir(this.getCacheDir(), function (err, files) {
    ...
    keysSync() {
        ...
        if (fs__default.existsSync(this.getCacheDir())) {
            return fs__default.readdirSync(this.getCacheDir()).map(self.transformFileNameToKey);
        }
        else {
            return [];
        }
    ...
    buildFilePath(name) {
        return path__default.normalize(this.getCacheDir() + '/' + name + '.json');
    }
